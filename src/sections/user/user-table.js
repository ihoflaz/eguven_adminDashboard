...
// function to handle close dialog
    ...
    const handleUrlFetch = async (userId) => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`http://localhost:3000/userUrl/${userId}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
        ...
        <Card>
                            ...
                            {items.map((user) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell>
                                            <Button onClick={() => handleUrlFetch(user.id)}>
                                                URL Al
                                            </Button>
                                        </TableCell>
                                    </TableRow>
            ...
            <Snackbar open={openSnackbar}
                ...
                <Alert onClose={() => setOpenSnackbar(false)}
                       severity="success"
                       variant="filled">
                    Kullanıcıya Ait URL Panoya Kopyalandı
                </Alert>
            </Snackbar>
        </Card>
    );
