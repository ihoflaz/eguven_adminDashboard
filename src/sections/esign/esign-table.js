...
export const EsignsTable = (props) => {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/orderDelete/${id}`, {
                method: 'POST',
                headers: {
            ...
            // After the request to /orderDelete/:id is successful, update the orders in the state
            setOrders(updatedOrders.esigns);
        } catch (error) {
            console.error(error);
    ...
    const handleConfirm = async (id) => {
        ...
        try {
            ...
            if (!response.ok) {
                console.error(`Failed to confirm order with id ${id}`);
                return;
            }
    ...
    const handleCheck = async (id) => {
        ...
        try {
            ...
            if (!response.ok) {
                console.error(`Failed to check order with id ${id}`);
                return;
            }
    ...
    return (
        <Card>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell>
                                            ...
                                            >
                                                {esign.status === 'Kontrol Ediliyor' ?
                                                    <CircularProgress size={24}/> : 'Kontrol Et'}
                                            </Button>
                                        </TableCell>
                                        <TableCell>
                                            <Button
                                                variant="contained"
                                                color="secondary"
                                                onClick={() => handleDeleteClick(esign)}
                                                disabled={esign.status !== 'Yeni'}
                                            >
                                                Sil
                                            </Button>
            ...
            >
                ...
                <DialogContent>
                    <Typography>
                        {deletingOrder && `Siparişi silmek istediğinizden emin misiniz? ${deletingOrder.firstName} ${deletingOrder.lastName}`}
                    </Typography>
                </DialogContent>
                <DialogActions>
